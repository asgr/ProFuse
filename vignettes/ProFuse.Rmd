---
title: "ProFound + ProFit + ProSpect = ProFuse"
author: "Aaron Robotham"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{ProFound + ProFit + ProSpect = ProFuse}
...
MultiFit = profuseMultiBandFound2Fit(image_list=image_list,
...
Now we actually run the fit with **Highlander**:
...
magcurve(massfunc_snorm_trunc(age=x,mSFR=10^highfit$parm["mSFR_1"],mpeak=highfit$parm["mpeak_1"],mperiod=10^highfit$parm["mperiod_1"],
  mskew=highfit$parm["mskew_1"], magemax=agemax/1e9),0,13.8e9,add=FALSE,col='red', ylim=c(0,2),xlab='Age (Yr)', ylab='SFR (Msol / Yr)')
magcurve(massfunc_snorm_trunc(age=x,mSFR=10^highfit$parm["mSFR_2"],mpeak=highfit$parm["mpeak_2"],mperiod=10^highfit$parm["mperiod_2"],
  mskew=highfit$parm["mskew_2"], magemax=agemax/1e9),0,13.8e9,add=TRUE,col='blue', ylim=c(0,2),xlab='Age (Yr)', ylab='SFR (Msol / Yr)')
legend('bottomright', legend=c('Bulge', 'Disk'), col=c('red', 'blue'), lty=1)
```

And we can check the final model image per band:

```{r fig.width=5, fig.height=5, dpi=100, eval=evalglobal}
profitLikeModel(highfit$parm, MultiFit, makeplots = TRUE)
```

We can also look at the SED per component, where as we might expect the bulge light is 'redder' than the disk light.

```{r fig.width=5, fig.height=5, dpi=100, eval=evalglobal}
bulgeSED = ParmOff(ProSpectSED, .args = c(as.list(highfit$parm[7:11]), MultiFit$data_ProSpect), .strip = '_1', .logged=c('mSFR', 'mperiod', 'Zfinal'))
diskSED = ParmOff(ProSpectSED, .args = c(as.list(highfit$parm[7:11+5]), MultiFit$data_ProSpect), .strip = '_2', .logged=c('mSFR', 'mperiod', 'Zfinal'))
